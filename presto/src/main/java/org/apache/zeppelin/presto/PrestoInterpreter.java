...
public class PrestoInterpreter extends Interpreter {
  ...
  static {
    Interpreter.register(
      ...
      new InterpreterPropertyBuilder()
        .add(PRESTOSERVER_URL, "jdbc:presto://localhost:8081/hive", "The URL for Presto")
  ...
  public InterpreterResult interpret(String cmd, InterpreterContext contextInterpreter) {
    ...
    // presto server need SQL query without ';'
    return executeSql(removeTrailingSemicolons(cmd));
  }

  private String removeTrailingSemicolons(String cmd) {
    if (cmd.endsWith(";")) {
      return removeTrailingSemicolons(cmd.substring(0, cmd.length() - 1));
    }
